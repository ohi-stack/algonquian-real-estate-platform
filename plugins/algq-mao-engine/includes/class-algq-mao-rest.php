<?php
/** REST API for calculations and authorized scenario reads. */
...
final class ALGQ_MAO_REST {
	...
	public function register() {
		register_rest_route( 'algq/v1', '/mao/calculate', array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'calculate' ), 'permission_callback' => '__return_true', 'args' => $this->args() ) );
		register_rest_route( 'algq/v1', '/mao/scenarios/(?P<id>\d+)', array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'scenario' ), 'permission_callback' => function () { return current_user_can( 'view_algq_underwriting' ); } ) );
	}
	public function calculate( $request ) {
		if ( ! $this->consume() ) { return new WP_Error( 'algq_mao_rate_limited', __( 'Calculation limit reached. Try again later.', 'algq-mao-engine' ), array( 'status' => 429 ) ); }
	...
	private function consume() {
		...
		$key = 'algq_mao_rate_' . hash( 'sha256', wp_salt( 'nonce' ) . '|' . $ip ); $count = (int) get_transient( $key );
		if ( $count >= 120 ) { return false; } set_transient( $key, $count + 1, HOUR_IN_SECONDS ); return true;
	}
}
