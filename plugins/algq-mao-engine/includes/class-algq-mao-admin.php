...
final class ALGQ_MAO_Admin {
	...
	public function dashboard() {
		...
		echo '<div class="wrap algq-mao-admin"><h1>Algonquian MAO Engine</h1><p>Versioned acquisition underwriting and approval control.</p><div class="algq-mao-kpis">';
		foreach ( array( 'Scenarios' => $m['total'], 'Approved' => $m['approved'], 'Average MAO' => '$' . number_format_i18n( $m['average_mao'], 0 ), 'High Risk' => $m['high_risk'] ) as $label => $value ) { echo '<div><strong>' . esc_html( $label ) . '</strong><span>' . esc_html( $value ) . '</span></div>'; }
		echo '</div></div>';
	}
	public function scenarios() {
		$this->cap( 'view_algq_underwriting' ); $rows = ALGQ_MAO_Database::recent();
		?><div class="wrap algq-mao-admin"><h1>Underwriting Scenarios</h1><?php if ( isset( $_GET['algq_mao_saved'] ) ) : ?><div class="notice notice-success"><p>Underwriting scenario saved as a draft.</p></div><?php endif; ?><table class="widefat striped algq-mao-table"><thead><tr><th>ID</th><th>Deal</th><th>Scenario</th><th>Strategy</th><th>MAO</th><th>Profit / NOI</th><th>Risk</th><th>Status</th><th>Created</th><th>Action</th></tr></thead><tbody><?php if ( ! $rows ) : ?><tr><td colspan="10">No scenarios saved.</td></tr><?php else : foreach ( $rows as $r ) : ?><tr><td><?php echo esc_html( $r->id ); ?></td><td><?php echo esc_html( $r->deal_id ?: '—' ); ?></td><td><?php echo esc_html( $r->scenario_name ?: 'Untitled' ); ?></td><td><?php echo esc_html( ucwords( $r->strategy ) ); ?></td><td><?php echo esc_html( '$' . number_format_i18n( $r->mao, 0 ) ); ?></td><td><?php echo esc_html( in_array( $r->strategy, array( 'rental','multifamily' ), true ) ? '$' . number_format_i18n( $r->noi, 0 ) . ' NOI' : '$' . number_format_i18n( $r->projected_profit, 0 ) ); ?></td><td><?php echo esc_html( $r->risk_flag ); ?></td><td><?php echo esc_html( ucfirst( $r->status ) ); ?></td><td><?php echo esc_html( $r->created_at ); ?></td><td><?php if ( 'draft' === $r->status && current_user_can( 'approve_algq_underwriting' ) ) : ?><a class="button" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=algq_mao_approve_underwriting&underwriting_id=' . absint( $r->id ) ), 'algq_mao_approve_' . absint( $r->id ) ) ); ?>">Approve</a><?php else : ?>—<?php endif; ?></td></tr><?php endforeach; endif; ?></tbody></table></div><?php
	}
	public function calculator_page() { $this->cap( 'manage_algq_underwriting' ); echo '<div class="wrap"><h1>MAO Calculator</h1>' . $this->engine->calculator_markup() . '</div>'; }
	public function settings_page() {
		$this->cap( 'manage_algq_mao_settings' ); $o = $this->calculator->assumptions(); $labels = array( 'assumption_version','wholesale_arv_multiplier','wholesale_closing_rate','flip_selling_cost_rate','rental_vacancy_rate','rental_reserve_rate','rental_target_cap_rate','repair_risk_threshold','minimum_profit_margin','default_holding_costs','default_financing_costs','default_desired_profit','default_assignment_fee' );
