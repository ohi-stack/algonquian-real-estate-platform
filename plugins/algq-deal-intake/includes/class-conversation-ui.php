<?php
/**
 * Production page-facing shortcode UI for Algonquian Deal Intake.
 *
 * Keeps canonical intake handlers and data ownership intact while ensuring each
 * shortcode renders a substantive Algonquian Real Estate interface.
 *
...
final class ALGQ_Deal_Intake_Conversation_UI {
	...
	public static function property_review(): string {
		$html  = self::shell_start(
			__( 'Property Owners • Property Review', 'algq-deal-intake' ),
			__( 'Request a Property Review', 'algq-deal-intake' ),
			__( 'Start with the property facts and your goals. This workflow helps identify the appropriate next review without requiring an immediate decision to sell.', 'algq-deal-intake' )
		);
		$html .= '<div class="algq-di-kpi-grid">';
		$html .= self::card( 'PROP', __( 'Property', 'algq-deal-intake' ), __( 'Address, type, occupancy, condition and relevant records.', 'algq-deal-intake' ) );
		$html .= self::card( 'SIT', __( 'Situation', 'algq-deal-intake' ), __( 'Repairs, vacancy, inheritance, carrying costs or transition concerns.', 'algq-deal-intake' ) );
		$html .= self::card( 'GOAL', __( 'Objectives', 'algq-deal-intake' ), __( 'Sell, keep, prepare, simplify or plan ahead.', 'algq-deal-intake' ) );
		$html .= self::card( 'NEXT', __( 'Direction', 'algq-deal-intake' ), __( 'Route the record to the appropriate acquisition or property-owner workflow.', 'algq-deal-intake' ) );
		$html .= '</div>';
		$html .= '<div class="algq-di-disclosure"><strong>' . esc_html__( 'Starting point, not a commitment.', 'algq-deal-intake' ) . '</strong><p>' . esc_html__( 'A property review is general information and intake. It is not a certified inspection, appraisal, legal or tax opinion, brokerage engagement, or commitment to purchase.', 'algq-deal-intake' ) . '</p></div>';
		$html .= '<div class="algq-di-workspace">' . ALGQ_Deal_Intake_Pages::public_form() . '</div>';
		$html .= self::shell_end();
		return $html;
	}
	...
	public static function homeowner_options(): string {
		$review = home_url( '/request-property-review/' );
	...
	public static function about(): string {
		...
		$html .= '<div class="algq-di-kpi-grid">' . self::card( '2.1', 'Release', 'Production-candidate source.' ) . self::card( 'INT', 'Authority', 'Seller and property intake evidence.' ) . self::card( 'CRM', 'Handoff', 'Pipeline CRM owns the canonical deal after acceptance.' ) . self::card( 'PDF', 'Artifacts', 'Protected submission files and intake PDF archive.' ) . '</div>';
		$html .= '<div class="algq-di-option-grid">';
		$html .= '<article class="algq-di-option"><h3>Core Functions</h3><p>Public and internal intake, property review, validation, consent evidence, duplicate review, lead scoring, supporting files, protected PDF records, audit events and controlled CRM handoff.</p></article>';
