...
final class ALGQ_Deal_Intake_Artifacts {
	...
	/**
	 * Add production upload support without duplicating the canonical form renderer.
	 */
	public static function enhance_form_output( string $output, string $tag, array|string $attr, array $match ): string { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
		...
		if ( false === stripos( $output, 'enctype=' ) ) {
			$output = preg_replace(
				'/<form\s+class="algq-di-form"\s+method="post"/i',
				'<form class="algq-di-form" method="post" enctype="multipart/form-data"',
				$output,
				1
			) ?? $output;
		}
		...
		if ( false === strpos( $output, 'name="' . self::UPLOAD_FIELD . '[]"' ) ) {
			$upload = '<div class="algq-di-upload algq-di-span-2">'
				. '<label>'
				. esc_html__( 'Supporting Files (optional)', 'algq-deal-intake' )
				. '<input type="file" name="' . esc_attr( self::UPLOAD_FIELD ) . '[]" multiple accept=".pdf,.jpg,.jpeg,.png,.webp,.docx">'
				. '<small>'
				. esc_html__( 'Upload property photos, a tax bill, lease, inspection, floor plan, or other relevant non-executable document. Maximum 8 files; 10 MB each by default.', 'algq-deal-intake' )
				. '</small>'
				. '</label>'
				. '</div>';

			$output = preg_replace(
				'/(<label\s+class="algq-di-consent">)/i',
				$upload . '$1',
				$output,
				1
			) ?? $output;
	...
	/**
	 * Verify Cloudflare Turnstile when deployment credentials are configured.
	 */
		...
		$response = wp_remote_post(
			...
			array(
				...
				'body' => array(
					...
					'remoteip' => ALGQ_Deal_Intake_Security::request_ip(),
				),
			)
		);
	...
	/**
	 * Create durable submission artifacts after the canonical intake transaction commits.
	 */
	public static function handle_submission_created( int $submission_id, array $record ): void {
		...
		if ( is_wp_error( $pdf ) ) {
			do_action(
				'algq_audit_event',
				'deal_intake.pdf_archive_failed',
				array(
					'submission_id' => $submission_id,
					'error' => $pdf->get_error_message(),
				)
			);
			return;
		}
	...
	/**
	 * Move optional public uploads into a protected Deal Intake storage area.
	 *
	...
	private static function process_uploaded_files( int $submission_id ): array {
		...
		foreach ( $files as $file ) {
			...
			if ( is_wp_error( $result ) ) {
				do_action(
					'algq_audit_event',
					'deal_intake.attachment_rejected',
					array(
						'submission_id' => $submission_id,
		...
		if ( $stored ) {
			do_action(
				'algq_audit_event',
				'deal_intake.attachments_stored',
				array(
					'submission_id' => $submission_id,
					'count' => count( $stored ),
				)
	...
	/**
	 * Normalize the multi-file $_FILES shape.
	 *
	 * @param array<string,mixed> $input Raw field from $_FILES.
	 * @return array<int,array<string,mixed>>
	 */
	...
	/**
	 * @param array<string,mixed> $file Normalized uploaded file.
	...
	private static function store_uploaded_file( int $submission_id, array $file ): array|WP_Error {
		...
		$inserted = $wpdb->insert(
			ALGQ_Deal_Intake_Database::table( 'attachments' ),
			array(
				'submission_id' => $submission_id,
	...
	/**
	 * @param array<int,array<string,mixed>> $uploaded Supporting files already stored.
	 * @return array<string,mixed>|WP_Error
	 */
	private static function create_submission_pdf( int $submission_id, array $record, array $uploaded ): array|WP_Error {
		...
		$attachment_id = 0;
		if ( get_option( 'algq_di_pdf_media_library_enabled', 1 ) ) {
			$attachment_id = self::register_pdf_attachment( $submission_id, $record, $path, $checksum );
		}
	...
	private static function register_pdf_attachment( int $submission_id, array $record, string $path, string $checksum ): int {
		$title = sprintf(
			/* translators: 1: intake reference, 2: property address */
			__( 'Deal Intake %1$s — %2$s', 'algq-deal-intake' ),
			(string) ( $record['uuid'] ?? $submission_id ),
	...
	/**
	 * @param array<int,array<string,mixed>> $uploaded Supporting files stored with the submission.
	 * @return array<int,string>
	 */
	private static function pdf_lines( array $record, array $uploaded ): array {
		$money = static function ( mixed $value ): string {
			$amount = (float) $value;
			return $amount > 0 ? '$' . number_format( $amount, 2 ) : 'Not provided';
		};

		$lines = array(
			'ALGONQUIAN REAL ESTATE LLC',
			'DEAL INTAKE SUBMISSION RECORD',
			'',
			'Reference: ' . (string) ( $record['uuid'] ?? '' ),
			'Submission ID: ' . (string) ( $record['id'] ?? '' ),
			'Received (UTC): ' . (string) ( $record['created_at'] ?? current_time( 'mysql', true ) ),
			'Status: ' . (string) ( $record['status'] ?? 'pending_review' ),
			'',
			'SELLER / CONTACT',
			'Name: ' . (string) ( $record['seller_name'] ?? '' ),
			'Email: ' . (string) ( $record['seller_email'] ?? '' ),
			'Phone: ' . (string) ( $record['seller_phone'] ?? '' ),
			'',
			'PROPERTY',
			'Address: ' . trim( (string) ( $record['address'] ?? '' ) . ', ' . (string) ( $record['city'] ?? '' ) . ', ' . (string) ( $record['state'] ?? '' ) . ' ' . (string) ( $record['postal_code'] ?? '' ), ', ' ),
			'Property type: ' . (string) ( $record['property_type'] ?? '' ),
			'Asking price: ' . $money( $record['asking_price'] ?? 0 ),
			'Timeline: ' . (string) ( $record['timeline'] ?? '' ),
			'Lead source: ' . (string) ( $record['lead_source'] ?? '' ),
			'Lead score: ' . (string) ( $record['lead_score'] ?? 0 ),
			'Duplicate status: ' . (string) ( $record['duplicate_status'] ?? '' ),
			'',
			'PROPERTY CONDITION / SITUATION',
			(string) ( $record['condition_summary'] ?? 'Not provided' ),
			'',
			'REASON FOR EXPLORING OPTIONS',
			(string) ( $record['motivation'] ?? 'Not provided' ),
			'',
			'SUPPORTING FILES',
			$uploaded ? (string) count( $uploaded ) . ' supporting file(s) stored in protected Deal Intake storage.' : 'No supporting files were uploaded.',
			'',
			'NOTICE',
			'This record documents an intake submission. It is not an appraisal, brokerage agreement, loan commitment, purchase contract, legal opinion, or commitment by Algonquian Real Estate LLC to acquire the property.',
		);

		return (array) apply_filters( 'algq_di_submission_pdf_lines', $lines, $record, $uploaded );
	}

	/**
	 * Generate a compact standards-compliant PDF without adding a third-party runtime dependency.
	 *
	 * @param array<int,string> $lines PDF text lines.
	 */
	...
	/**
	 * @param array<string,mixed> $pdf Generated PDF metadata.
	 * @param array<int,array<string,mixed>> $uploaded Supporting file metadata.
	 */
	private static function send_archive_email( int $submission_id, array $record, array $pdf, array $uploaded ): void {
		...
		if ( ! is_email( $email ) || empty( $pdf['path'] ) || ! is_readable( (string) $pdf['path'] ) ) {
			return;
		}

		$subject = sprintf(
			'Algonquian Deal Intake PDF — %s — %s',
			(string) ( $record['uuid'] ?? $submission_id ),
			(string) ( $record['address'] ?? '' )
		);
		$message = implode(
			"\n",
			array(
				'A new Algonquian Deal Intake submission has been archived as PDF.',
				'',
				'Reference: ' . (string) ( $record['uuid'] ?? '' ),
				'Submission ID: ' . $submission_id,
				'Property: ' . (string) ( $record['address'] ?? '' ) . ', ' . (string) ( $record['city'] ?? '' ) . ', ' . (string) ( $record['state'] ?? '' ),
				'Seller: ' . (string) ( $record['seller_name'] ?? '' ),
				'Lead score: ' . (string) ( $record['lead_score'] ?? 0 ),
				'Supporting files stored: ' . count( $uploaded ),
				'PDF SHA-256: ' . (string) ( $pdf['checksum'] ?? '' ),
		...
		$sent = wp_mail(
			...
			array( (string) $pdf['path'] )
		);

		do_action(
			'algq_audit_event',
			$sent ? 'deal_intake.pdf_archive_emailed' : 'deal_intake.pdf_archive_email_failed',
			array(
				'submission_id' => $submission_id,
				'to' => $email,
				'attachment_id' => (int) ( $pdf['attachment_id'] ?? 0 ),
			)
		);
	}
	...
	private static function private_storage_root(): string {
		...
		$default = trailingslashit( (string) $uploads['basedir'] ) . 'algq-private/deal-intake';
		$root = (string) apply_filters( 'algq_di_private_storage_dir', $default );
	...
	public static function protected_attachment_url( string $url, int $attachment_id ): string {
		if ( 1 !== absint( get_post_meta( $attachment_id, self::PRIVATE_ATTACHMENT_META, true ) ) ) {
			return $url;
		}
		...
		return add_query_arg(
			array(
				'action' => 'algq_di_download_artifact',
				'attachment_id' => $attachment_id,
				'token' => self::artifact_token( $attachment_id, $file ),
			),
	...
	public static function download_artifact(): void {
		if ( ! current_user_can( ALGQ_Deal_Intake_Security::CAP_VIEW_PRIVATE ) ) {
			wp_die( esc_html__( 'You do not have permission to download this intake artifact.', 'algq-deal-intake' ), '', array( 'response' => 403 ) );
		}

		$attachment_id = isset( $_GET['attachment_id'] ) ? absint( $_GET['attachment_id'] ) : 0;
		$token = isset( $_GET['token'] ) ? sanitize_text_field( wp_unslash( $_GET['token'] ) ) : '';
		$file = $attachment_id ? get_attached_file( $attachment_id ) : '';
		if ( ! $attachment_id || ! $file || ! hash_equals( self::artifact_token( $attachment_id, $file ), $token ) ) {
			wp_die( esc_html__( 'The intake artifact request is invalid.', 'algq-deal-intake' ), '', array( 'response' => 403 ) );
		}

		$root_real = realpath( self::private_storage_root() );
		$file_real = realpath( $file );
		if ( false === $root_real || false === $file_real || 0 !== strpos( wp_normalize_path( $file_real ), trailingslashit( wp_normalize_path( $root_real ) ) ) || ! is_readable( $file_real ) ) {
			wp_die( esc_html__( 'The intake artifact is unavailable.', 'algq-deal-intake' ), '', array( 'response' => 404 ) );
		}
	...
	private static function artifact_token( int $attachment_id, string $file ): string {
		return hash_hmac( 'sha256', $attachment_id . '|' . wp_normalize_path( $file ), wp_salt( 'auth' ) );
	}
}
