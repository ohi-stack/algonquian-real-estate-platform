 ...
 * Description: Creates versioned acquisition proposals and offers from approved deal and underwriting data, manages review and approval, and delegates document and PDF execution to Algonquian platform services.
 * Version: 2.1.0
...
define( 'ALGQ_OFFER_VERSION', '2.1.0' );
define( 'ALGQ_OFFER_DB_VERSION', '2.1.0' );
...
$algq_offer_includes = array(
    ...
    'class-seller-financing-proposals.php',
...
function algq_offer_deactivate(): void { flush_rewrite_rules(); }
...
function algq_offer_init(): void {
    ...
    $classes = array(
        'ALGQ_Offer_Post_Types', 'ALGQ_Offer_Shortcodes', 'ALGQ_Offer_Builder',
        'ALGQ_Offer_Template_Manager', 'ALGQ_Offer_Document_Generator', 'ALGQ_Offer_PDF_Integration',
        'ALGQ_Offer_Deal_Integration', 'ALGQ_Offer_Automation_Hooks', 'ALGQ_Offer_Admin',
        'ALGQ_Offer_Audit_Log', 'ALGQ_Offer_Role_Capabilities', 'ALGQ_Offer_REST_API', 'ALGQ_Offer_Settings',
    );
...
function algq_offer_dependency_notice(): void {
    ...
    if ( $missing ) {
        printf( '<div class="notice notice-warning"><p><strong>%1$s</strong> %2$s</p></div>', esc_html__( 'Algonquian Offer Generator:', 'algq-offer-generator' ), esc_html( sprintf( __( 'limited mode is active because these dependencies were not detected: %s.', 'algq-offer-generator' ), implode( ', ', $missing ) ) ) );
    }
}
...
function algq_offer_enqueue_front_assets(): void {
    ...
    wp_localize_script( 'algq-offer-generator', 'algqOfferGenerator', array( 'ajaxUrl' => admin_url( 'admin-ajax.php' ), 'nonce' => wp_create_nonce( 'algq_offer_generator' ) ) );
}
add_action( 'wp_enqueue_scripts', 'algq_offer_enqueue_front_assets' );

function algq_offer_enqueue_admin_assets( string $hook ): void {
    if ( false === strpos( $hook, 'algq-offer' ) && false === strpos( $hook, 'algq_offer' ) ) { return; }
