...
final class ALGQ_Offer_REST_API {
    ...
    public static function routes(): void {
        ...
        register_rest_route( 'algq/v1', '/offers/(?P<id>\d+)/approve', array(
            'methods' => WP_REST_Server::CREATABLE, 'callback' => array( __CLASS__, 'approve' ), 'permission_callback' => static fn() => current_user_can( 'approve_algq_offers' ),
        ) );
        register_rest_route( 'algq/v1', '/offers/(?P<id>\d+)/document', array(
            'methods' => WP_REST_Server::CREATABLE, 'callback' => array( __CLASS__, 'document' ), 'permission_callback' => static fn() => current_user_can( 'generate_algq_offer_documents' ),
        ) );
    }
    ...
    public static function index( WP_REST_Request $request ): WP_REST_Response {
        ...
        $items    = array_map( array( __CLASS__, 'prepare' ), $query->posts );
        return new WP_REST_Response( array( 'items' => $items, 'page' => $page, 'pages' => (int) $query->max_num_pages, 'total' => (int) $query->found_posts ) );
    }

    public static function create( WP_REST_Request $request ) {
        $result = ALGQ_Offer_Service::create( $request->get_json_params() ?: $request->get_params(), get_current_user_id() );
        return is_wp_error( $result ) ? $result : new WP_REST_Response( self::prepare( get_post( $result ) ), 201 );
    }
    ...
    public static function document( WP_REST_Request $request ) {
        ...
        if ( 'algq_offer' !== get_post_type( $offer_id ) ) {
            return new WP_Error( 'algq_offer_not_found', __( 'Offer not found.', 'algq-offer-generator' ), array( 'status' => 404 ) );
        }
        ...
        update_post_meta( $offer_id, '_algq_offer_document_hash', hash( 'sha256', $html ) );
        do_action( 'algq_offer_document_generated', $offer_id, get_current_user_id() );
        return rest_ensure_response( array( 'offer_id' => $offer_id, 'hash' => hash( 'sha256', $html ) ) );
    }
    ...
    private static function prepare( WP_Post $post, bool $detailed = false ): array {
        ...
        $out  = array(
            ...
            'workflow_status' => $data['offer_status'] ?: get_post_status( $post ), 'strategy' => $data['strategy'],
            'purchase_price' => $data['purchase_price'], 'deal_id' => absint( $data['deal_id'] ),
            'version' => absint( $data['version_number'] ?: 1 ), 'modified_gmt' => get_post_modified_time( 'c', true, $post ),
        );
        if ( $detailed ) {
            $out['details'] = $data;
            $out['document_hash'] = get_post_meta( $post->ID, '_algq_offer_document_hash', true );
        }
        return $out;
    }
}
