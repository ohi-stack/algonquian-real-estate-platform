 ...
 * Description: Executive transaction control, decisions, exceptions, revenue, agent operations, cross-plugin intelligence, platform health, audit visibility, secured reporting, and system command controls for the Algonquian Real Estate Platform.
 * Version: 1.3.0
 * Author: Onegodian
...
define( 'ALGQ_COMMAND_CENTER_VERSION', '1.3.0' );
...
add_action( 'plugins_loaded', static function (): void {
    load_plugin_textdomain( 'algq-command-center', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
    if ( version_compare( PHP_VERSION, ALGQ_COMMAND_CENTER_MIN_PHP, '<' ) ) {
        add_action( 'admin_notices', static function (): void { echo '<div class="notice notice-error"><p>' . esc_html__( 'Algonquian Admin Command Center requires PHP 8.2 or newer.', 'algq-command-center' ) . '</p></div>'; } );
        return;
    }
    ( new ALGQ_Command_Center() )->run();
} );
